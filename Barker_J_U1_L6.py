...
def make_graph(nodes="rrNodes", node_city="rrNodeCity.txt", edges="rrEdges.txt"):
    ...
    with open(edges, "r") as rrNodes:
        ...
        for line in lines:
            ...
            edgeCost[(liner[1], liner[0])] = calc_edge_cost(*nodeLoc[liner[0]], *nodeLoc[liner[1]])
...
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
    ...
    while frontier:
        ...
        if current == goal:
            ...
            return cc[2], sum([calc_edge_cost(*graph[0][cc[2][x]], *graph[0][cc[2][x+1]]) for x in range(len(cc[2])-2)])
        ...
        for child in graph[3][current]:
            ...
            pp.extend(cc[2])
            pp.append(child)
            f = heuristic(child, goal, graph) + sum([calc_edge_cost(*graph[0][pp[x]], *graph[0][pp[x+1]]) for x in range(len(pp)-2)])
